IF (OBJECT_ID('FOX_GET_SMART_ORDERING_SOURCE_RFO') IS NOT NULL ) DROP PROCEDURE FOX_GET_SMART_ORDERING_SOURCE_RFO  
GO 
--[dbo].FOX_GET_SMART_ORDERING_SOURCE 1011163,'TES'        
CREATE PROCEDURE [dbo].FOX_GET_SMART_ORDERING_SOURCE_RFO      
...
AS                
     BEGIN                
        ...
        id4.NAME AS HHH_DESCRIPTION              
         ...
         WHERE            
   (LTRIM(RTRIM(s.FIRST_NAME)) LIKE '%'+@SEARCHVALUE+'%'                
               OR LTRIM(RTRIM(s.LAST_NAME)) LIKE '%'+@SEARCHVALUE+'%'                
               OR LTRIM(RTRIM(s.FIRST_NAME))+' '+LTRIM(RTRIM(s.LAST_NAME)) LIKE '%'+@SEARCHVALUE+'%'                
 OR LTRIM(RTRIM(s.FIRST_NAME))+' '+LTRIM(RTRIM(s.LAST_NAME))+' '+LTRIM(RTRIM(S.REFERRAL_REGION)) LIKE '%'+@SEARCHVALUE+'%'                
  ...
  AND s.NPI  NOT LIKE '0000%'             
  AND (ISNULL(s.CODE, '') ='' OR  s.CODE <> '00000')             
  AND s.FIRST_NAME NOT LIKE '%unknown%'              
  AND s.LAST_NAME NOT like  '%unknown%' ;              
